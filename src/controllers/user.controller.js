...
const registerUser = asyncHandler( async (req, res) => {
    ...
    //like this if-else condition we can check all this thing for fullname, email, username, password
    // if(fullname==""){
    //     throw new apiError(400, "fullname is required")
    // }

    //by this way, using .some() fn check for all fullname, email, username, password
    if([fullname, email, username, password].some((field) => field?.trim() === ""))
    {
        throw new apiError(400, "All field are required");
    }

    //by this way, check one by one
    //const existedEmail = User.findOne({email})
    //const existedName = User.findOne({username})
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    return res.status(200)
    .json( new apiResponse(400, "Avatar Image updated successfully"))
})
